...
contract MerkleProof {
    ...
    mapping(bytes32 => bool) claimedLeaves;
...
function claim(bytes32 leaf,bytes32[] memory proof,bytes32 addressHash,address account, uint256 amount) public{

    require(claimIsActive, "Claim is not enabled");
    require(!claimedAddresses[account], "MerkleDistributor: Drop already claimed.");
    require(!claimedLeaves[leaf], "MerkleDistributor: Leaf already claimed.");
    require(msg.sender==account, "Sender not claimer");
    require(hash(amount,account)==addressHash, "Address Hash Failed");
    require(verify(leaf,proof), "Not Eligible");

    mnst.transferFrom(distributionWallet,account,amount);  //transfer
